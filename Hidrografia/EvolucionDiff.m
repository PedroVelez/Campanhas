...
if exist('campanha','var')==0; DC.campanha='';end
...
title(sprintf('Mean (%4d-bottom) Sal',presavg));xlabel('Fecha')
...
for i1=1:5:length(nstats)
    if dates(i1)>floor(min(dates(isnan(difsalt)==0))) & dates(i1)<ceil(max(dates(isnan(difsalt)==0)))
        text(dates(i1),nanmin(msalts),num2str(nstats(i1)),'color','k','VerticalAlignment','top','fontsize',12);hold on
    end
end
legend([h1s h2s],'Sal','Sal2','Location','NorthWest')
%plot([dates(nstats==EstacionCambioCTD)-2/24 dates(nstats==EstacionCambioCTD)-2/24],[min(get(gca,'YTick')) max(get(gca,'YTick'))],'color','b','linewidth',2.75);

subplot(2,1,2)
plot([floor(min(dates(isnan(difsalt)==0)))   ceil(max(dates(isnan(difsalt)==0)))],[0 0],'--','color',[0.65 0.65 0.65],'linewidth',2);grid on;hold on
errorbar(dates,difsalt,stdifsalt,'k','linewidth',1)
plot(dates,difsalt,'k','linewidth',2.25);
title(sprintf('Promedio (%4d-bottom) Sal-Sal2',prescomp));xlabel('Fecha')
axis([floor(min(dates(isnan(difsalt)==0)))   ceil(max(dates(isnan(difsalt)==0))) -inf inf])
datetick('keeplimits')
for i1=1:5:length(nstats)
    if dates(i1)>floor(min(dates(isnan(difsalt)==0))) & dates(i1)<ceil(max(dates(isnan(difsalt)==0)))
...
title(sprintf('Promedio (%4d-bottom) Tem',presavg));xlabel('Fecha')
axis([floor(min(dates(isnan(difsalt)==0)))   ceil(max(dates(isnan(difsalt)==0))) -inf inf])
datetick('keeplimits')
for i1=1:5:length(nstats)
    if dates(i1)>floor(min(dates(isnan(difsalt)==0))) & dates(i1)<ceil(max(dates(isnan(difsalt)==0)))
...
subplot(2,1,2)
plot([floor(min(dates(isnan(diftemp)==0)))   ceil(max(dates(isnan(diftemp)==0)))],[0 0],'--','color',[0.65 0.65 0.65],'linewidth',2);grid on;hold on
errorbar(dates,diftemp,stdiftemp,'k');
plot(dates,diftemp,'k','linewidth',2.25);
title(sprintf('Promedio (%4d-bottom) Tem-Tem2',prescomp));xlabel('Fecha')
axis([floor(min(dates(isnan(difsalt)==0)))   ceil(max(dates(isnan(difsalt)==0))) -inf inf])
datetick('keeplimits')
for i1=1:5:length(nstats)
