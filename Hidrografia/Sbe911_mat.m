...
while (~strncmp(str,'*END*',5))
    ...
    elseif contains(str,'* NMEA Latitude')
        is=strfind(str,'=');
        dm=sscanf(str(is+1:end),'%d %f %1s');
    ...
    elseif contains(str,'* NMEA Longitude')
        is=strfind(str,'=');
        dm=sscanf(str(is+1:end),'%d %f %s');
    ...
    elseif contains(str,'* NMEA UTC (Time)')
        is=strfind(str,'=');
        datstr=[str(is+6:is+7) '-' str(is+2:is+4) '-' str(is+9:is+12)];
        n=datenum(datstr);
        gtimeNMEA=datevec(n);
        iss=strfind(str,':');
        gtimeNMEA(4)=str2double(str(iss(1)-2:iss(1)-1));
        gtimeNMEA(5)=str2double(str(iss(2)-2:iss(2)-1));
        gtimeNMEA(6)=str2double(str(iss(2)+1:iss(2)+2));
    elseif contains(str,'**Depth')
        is=strfind(str,':');
        dm=str2double(str(is+1:end));
        if isempty(dm)
            depth=NaN;
    ...
    elseif contains(str,'**Bottom Depth [m]:') && exist('depth')==0
        is=strfind(str,':');
