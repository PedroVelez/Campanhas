...
%  Despues calcula variables derivadas y lo pasa a formato .mat con el mismo nombre que el fichero
%   de entrada.
...
if ~isempty(Files)
    while r==2
        ...
        for ivar=1:size(names,1)
            fprintf('    >Variables %s \n',names(ivar,:));
            if contains(names(ivar,:),'Pressure, Digiquartz [db]')
                pres=data(:,ivar);
            end
        end
        presraw=pres;
        ...
        for ivar=1:size(names,1)
            if contains(names(ivar,:),'Salinity, Practical [PSU]')
                saltraw=data(:,ivar);
            ...
            if contains(names(ivar,:),'Salinity, Practical, 2 [PSU]')
            ...
            if contains(names(ivar,:),'Temperature [ITS-90, deg C]')
                tempraw=data(:,ivar);
                temp=data(1:iPresMax,ivar);
            end
            if contains(names(ivar,:),'Temperature, 2 [ITS-90, deg C]')
                temp2=data(1:iPresMax,ivar);
            end
            if contains(names(ivar,:),'Fluorescence, WET Labs ECO-AFL/FL [mg/m^3]')
                flu=data(1:iPresMax,ivar);
            end
            if contains(names(ivar,:),'Oxygen, SBE 43 [umol/kg]')
                oxyi=data(1:iPresMax,ivar);
            end
            if contains(names(ivar,:),'Oxygen, SBE 43, 2 [umol/kg]')
                oxyi2=data(1:iPresMax,ivar);
            end
        end
        ...
        if exist('oxyi2','var')
            oxyi2=interp1(pres,oxyi2,(Presmin:2:max(pres))');
        end
        ...
        gama=eos80_legacy_gamma_n(salt,temp,pres,long,lati);
        ...
        if Automatico==0
            ...
            title(sprintf('File: %s, estacion:%3.0d, \n Lat:%5.3f, Lon:%5.3f, depth(dbar): %4.1f \n Date:%s', ...
                file,nstat,lati,long,depth, ...
                datestr(datenum(gtime(1),gtime(2),gtime(3),gtime(4),gtime(5),gtime(6)))), ...
                'interpreter','none');
            m_line(long,lati,'marker','o','markersize',6,'color','b','MarkerFaceColor','b');
            m_text(long-0.15,lati+0.1,num2str(nstat), ...
        ...
        LATI=lati;
        LONG=long+360;
        save(file(1:length(file)-4),'long','lati','nstat','gtime','depth','names', ...
            'pres','temp','salt','sgth','ptmp','pote','gama','dynh',...
            'SALT','TEMP','PRES','DYNH','POTE','PTMP','SGTH','GAMA','LATI','LONG')
        if exist('temp2','var')
        ...
        if exist('oxyi2','var')
            save(file(1:length(file)-4),'-append','oxyi2','OXYI2')
        end
        ...
        if Automatico==0
            ...
            figure(1)
            if np<length(Files)
                r=input('>>>>> Seguimos procesando (2 [Yes and move .mat files] / 1 [yes] / 0 [no])?');
                if r == 0
                    break;
                end
                if r==2
                    movefile(strcat(file(1:length(file)-4),'.mat'), ...
                        strcat('../../CTD/Mat/',file(1:length(file)-4),'.mat'));
                end
                r=2;
