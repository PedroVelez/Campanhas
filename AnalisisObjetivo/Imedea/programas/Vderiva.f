...
C ====================================================================
C
C   THIS CODE COMPUTES SEVERAL DERIVED VARIABLES FROM GRIDDED VALUES 
C   OF OBSERVED VARIABLES (THE OUTPUTS OF ANÁLISIS.F) BY MEANS OF A 
C   FINITTE DIFERENCES SCHEME.
C
C ====================================================================
C           
...
C   - the user selects between computing the analysis just at one 
...
C   - input files with the grid values of the corresponding observed 
C     variable are read
C   - grid values of the derived variable are computed by finitte
C     differences 
C   - they are finally written in one ore the whole set of files.
C
C   External files used:  
C   - INPUT files:
C     * domain info file:                           [../info/grid.dat]
C     * analysis of observed variables:        [../snp/NNNNLLLLVV.snp]  
C                                           or [../grd/NNNNLLLLVV.grd] 
C   - OUPUT files:
C     * field(s) of derived variables:        [../snp/NNNNLLLLVVV.snp] 
C                                          or [../grd/NNNNLLLLVVV.grd] 
C     * mean level density (only for the 
C       option of the Vertical Forcing):        [../info/NNNN_st0.dat] 
C       
C   Built-in routines:  POLFIT.F, FINDIF.F
