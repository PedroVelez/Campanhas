...
C ====================================================================
C
C   THIS CODE SOLVES THE QUASI-GEOSTROPHIC OMEGA EQUATION [GIVING 
C   THE VERTICAL VELOCITY ON OUTPUT] ina 3D DOMAIN BY MEANS OF A 
C   RELAXATION METHOD. (X,Y,Z) COORDINATES ARE ASSUMED.
C
C ====================================================================
C           
...
C
C   Work structure:
C   - reads information on the grid from the domain file 'grid.dat'
C   - selects a few options and reads a few inversion parameters from 
C     the external file 'ominput.dat' (e.g., boundary conditions).
C   - reads the whole set of 2D files conforming the 3D grid and 
C     containing the vertical forcing grid values
C   - reads the file containing the mean density at every level 
C     '*_st0.dat' (requested to compute the Brunt-Vaissalla frequency).
...
C     * vert.forcing gridpoint data files:    [../snp/NNNNLLLLqdi.snp] 
C                                          or [../grd/NNNNLLLLqdi.grd] 
C     * mean level density:                     [../info/NNNN_st0.dat] 
C     * inversion parameters:                    [../info/ominput.dat]
C     * if NBOUND=2: file with distributions of
C       w at the (6) 2D boundaries of the domain:    [../snp/ombc.snp] 
C     * if NBOUND=3: bathimetry files:              [../snp/batim.snp]
C   - OUPUT files:
C     * vert.velocity gridpoint data files:   [../snp/NNNNLLLLdhw.snp] 
C                                          or [../grd/NNNNLLLLdhw.grd] 
C
...
C   they are all are expressed in SI units
C
...
C ====================================================================
      PARAMETER (NLMAX=100)
	REAL FORZ(0:NLMAX,0:NLMAX,0:NLMAX)
	REAL N22(0:NLMAX,0:NLMAX,0:NLMAX),N2(0:NLMAX)
      REAL UU(0:NLMAX,0:NLMAX,0:NLMAX)
	REAL VV(0:NLMAX,0:NLMAX,0:NLMAX)
      REAL W(0:NLMAX,0:NLMAX,0:NLMAX)
	REAL ZW(0:NLMAX,0:NLMAX,0:NLMAX)
      REAL ROM(0:NLMAX),ROMU(0:NLMAX),COEF(0:NLMAX)
	REAL C1(0:NLMAX),C2(0:NLMAX)
      REAL VMIN(0:NLMAX),VMAX(0:NLMAX)
      REAL N2PARAM
      INTEGER NBAT(0:NLMAX,0:NLMAX)
	INTEGER NBAT_(0:NLMAX,0:NLMAX)
      CHARACTER*4,NA1
      CHARACTER*4,NA0,ME
      CHARACTER*8,NA(0:NLMAX)
      CHARACTER*20,LIN(5)
C
      G=9.81
      PI=3.14159
