...
C     * level data files:                      [../level/NNNNLLLL.lev]
C   - OUTPUT files: 
C     * gridpoint data files:       [../varobs/NNNNLLLLVV.snp or .grd] 
C
...
C ====================================================================
      ...
      PARAMETER(NS=750,NG=2100,NLMAX=100)
...
C ======================================================================
      WRITE(6,*)
	WRITE(6,32) 
   32 FORMAT(1X,'*** The code can operate on a single level (op. 1) '
     .      ,'or for'/1X,'*** the whole set of levels (op.2)')  
   34 WRITE(6,'(a,$)') ' >>> SELECT OPTION: '
   	READ(5,*,ERR=34) NREP
  	IF(NREP.EQ.1) THEN
C        Single level option
         ILFIN=1
   36    WRITE(6,'(a,$)')' >>> Input level depth (m): '
         READ(5,*,ERR=36) DEP
         LEV=DEP+0.00001
         NA(1)=NA0//CHAR(LEV/1000+48)//CHAR(LEV/100-10*(LEV/1000)+48)
     .	   //CHAR(LEV/10-10*(LEV/100)+48)//CHAR(LEV-10*(LEV/10)+48)
      ELSE
C        Multi-level option
         ILFIN=NL
         DO I=1,NL
		  LEV=(P0+PINT*FLOAT(I-1)+0.00001)
            NA(I)=NA0//CHAR(LEV/1000+48)//CHAR(LEV/100-10*(LEV/1000)+48)
     .	      //CHAR(LEV/10-10*(LEV/100)+48)//CHAR(LEV-10*(LEV/10)+48)
         ENDDO
      ENDIF
C
   	WRITE(6,40)
   40 FORMAT(//1X,'*** VARIABLES:   1: temp   2: sal   3: st    4: dh',
     ./18X,'5: th     6: u     7: v     8: pres  9: mp')  
   42 WRITE(6,'(a,$)') ' >>> CHOOSE variable: '
      READ(5,*,ERR=42) NVAR
C	
      IL=1
   44 NND=0
...
C           the value of 'no data' for pressure is -99.99
   ...
   77 NSTA=ND-1
	...
	WRITE(6,93) NA(IL)
      ...
      WRITE(6,98) NSTA
   93	FORMAT(//5X,'#########    FILE: ',A8,'.LEV',6X,'#########')
   94 FORMAT(5X,'###',7X,'READ ',I3,' STATIONS FROM THE FILE  ###')
...
C
  100 IF(IL.EQ.1) THEN
         WRITE(6,102)
  102    FORMAT(//,1X,'*** INTERPOLATION ROUTINES AVAILABLE:'
     .   /5X,'1- OPTIMAL STATISTICAL INTERPOLATION (OSI)'
     .   /5X,'2- SUCCESSIVE CORRECTIONS (->OSI)')
C    .   /5X,'3- STANDARD VERSION OF SUCCESSIVE CORRECTIONS')
  105    WRITE(6,'(a,$)') ' >>> Choose option: '
         READ(5,*,ERR=105) NRUT
	ENDIF
	IF(NRUT.EQ.1) CALL OSI(NSTA,NGRI,IL)
	IF(NRUT.EQ.2) CALL SUCCOR(NRUT,NSTA,NGRI,IL)
C
...
C ======================================================================
  ...
  123    WRITE(6,'(a,$)') ' >>> Format option: 1-SURFER  2-SNP : '
         READ(5,*,ERR=123) NSOR
         WRITE(6,125)
  125    FORMAT(1X,'>>> Save complementary information (about the ',
     ...
     .   '     the data body of the output file (Y/[N]): '
      ...
      ENDIF
C
      OPEN(unit=3,file='../varobs/'//na(IL)//icvar(nvar)//ifor(nsor),
     .                                             status='unknown')
      IF(NSOR.EQ.1) THEN
C        Data plotted in SURFER (.grd) format
         IF(abs(alf0).gt.0.) THEN
            xmin=0.
            ymin=0.
         ELSE
            xmin=xlon1
...
C
      IF(NOP.EQ.'Y') THEN
         ...
         WRITE(3,147) XLAT2,XLON2,XLAT3,XLON3,XLAT1,XLON1,XLAT4,XLON4
         IF (ABS(ALF0).GT.0.1) THEN
...
C        Values at station points:
  ...
  150    FORMAT(1X,'Grid:',7X,I2,' X ',I2,' points, with XARM =',F5.1,
     .     ' km, YARM =',F5.1,' km')
  152    FORMAT(1X,'Grid:',7X,I2,' X ',I2,' points, with XARM =',F5.3,
     .     ' deg, YARM =',F5.3,' deg')
  155    FORMAT(//1X,'Analysis technique: OSI. Parameters used:'/1X,'POL
     .     INOM.DEG=',F2.0,'  SCL=',F4.0,'  GAMMA=',F6.4,'  XCUT=',F4.0)
  157    FORMAT(//1X,'Analysis technique: SC-Standard. Parameters used:'
     .     /1X,'DETR.DEGREE=',F2.0,'  SCL0=',F4.0,'  G=',F4.2,'  NITER='
     .     ,F4.0)
  160    FORMAT(//1X,'Analysis technique: SC->OSI. Parameters used:'
     .     /1X,'DETR.DEGREE=',F2.0,'  SCL=',F4.0,'  GAMMA=',
     .      F6.4,'  XCUT=',F4.0,'  NITER=',F4.0)
  163    FORMAT(/1x,'Data:'//1X,'SYMBOL    LATITUD  LONGITUD   OBS.DATA'
     .     ,'   ANA.DATA')
...
C
	IL=IL+1 
	IF(IL.LE.ILFIN) GOTO 44
      IF(ILFIN.EQ.1) THEN
	   WRITE(6,*)
	   WRITE(6,'(a,$)') ' >>> Change analysis parameters ?  (Y/[N]) '
         READ(5,13) NOP
	   IL=1
...
C ======================================================================
C                     SUBROUTINE SUCCOR (NRUT,NSTA,NGRI,IL)
C
C   Work structure:
C   - detrends the data
...
C
C ======================================================================
      SUBROUTINE SUCCOR(NRUT,NSTA,NGRI,IL)
      IMPLICIT REAL*8 (A-H,O-Z)
      PARAMETER(NS=750,NG=2100,ND=2)
      COMMON/INOUT/X(NS),Y(NS),V(NS),VS(NS),XG(NG),YG(NG),VG(NG),PAR(5)
      COMMON/DET/SS(ND,NS),FIN(NS),DIN(NS),SG(ND,NG),TR(NG)
C ----------------------------------------------------------------------
      IF(IL.EQ.1) THEN
         WRITE(6,3)
    3    FORMAT(10(/),1X,'############# SUCCESSIVE CORRECTIONS ',
     .   'ANALYSIS METHOD  #############'//)
      ENDIF
C
C     PRELIMINARY STEPS: 
...
C ======================================================================
  	IF(IL.EQ.1) THEN
         WRITE(6,13)
   13    FORMAT(/1X,'*** Distance Weight Functions are gausians:'
     .   /14X,'W = EXP(-d**2/2*Sn**2)'/5X,'where Sn is the characteris',
     .   'tic scale used in the'/5X,'n-iteration and d is distance.'/)
   15    WRITE(6,'(a,$)') ' >>> Number of iterations  [NITER<100]: '
         READ(5,*,ERR=15) NITER
         PAR(5)=NITER
      ELSE
	   NITER=PAR(5)
...
C        SC - STANDARD:
  	   IF(IL.EQ.1) THEN
            WRITE(6,17)
   17       FORMAT(/1X,'*** For the STANDARD VERSION of SC, Sn is ', 
     .      'taken as'/5X,'Sn=G*S(n-1), with 0<G<1. So you must:')
...
C        SC - OSI:
   ...
   30      WRITE(6,'(a,$)') 
     .     ' >>> The noise to signal variance ratio  [gamma] : '
           ...
           PAR(3)=GAMMA
           WRITE(6,32)
   32	     FORMAT(' >>> The cut-off wavelength [XCUT(km)] for spatial')
   33      WRITE(6,'(a,$)') 
...
C ======================================================================     
   ...
   20    WRITE(6,'(a,$)') 
     .   ' >>> The noise to signal variance ratio  [gamma] : '
         READ(5,*,ERR=20) GAMMA
         PAR(3)=GAMMA
         WRITE(6,*) '>>> The cut-off wavelength [XCUT(km)] for spatial'
   23    WRITE(6,'(a,$)') 
     .   '     smoothing (set to 0 for no additional smoothing): '
         READ(5,*,ERR=23) XCUT
         PAR(4)=XCUT
...
C     Attempts spectral decomposition:
      do i=1,nsta
         ...
         enddo
      enddo
      call jacobi(a_,nsta,ns,eval,ev,nrot)
c     xx=0.
c     do i=1,nsta
c        xx=xx+eval(i)
c        type*,i,eval(i),xx
...
C ======================================================================
      ...
      IMPLICIT REAL*8 (A-H,O-Z)
      PARAMETER(NS=750,NG=2100,ND=2,ID=3)
      COMMON/INOUT/X(NS),Y(NS),V(NS),VS(NS),XG(NG),YG(NG),VG(NG),PAR(5)
...
c     --------------------------------------
      ...
      IMPLICIT REAL*8 (A-H,O-Z)
      PARAMETER(NS=750,NG=2100,ND=2,ID=50)
      COMMON/INOUT/X(NS),Y(NS),V(NS),VS(NS),XG(NG),YG(NG),VG(NG),PAR(5)
      COMMON/DET/SS(ND,NS),ZB(NS),Z(NS),SG(ND,NG),TR(NG)
